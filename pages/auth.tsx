				...
				<div className='flex justify-center'>
					<div className='self-center w-full px-16 py-16 mt-2 bg-black rounded-md bg-opacity-70 lg:w-2/5 lg:max-w-md'>
						...
						<div className='flex flex-col gap-4'>
							{variant === "register" && <Input id='name' type='text' label='Username' value={name} onChange={(e: any) => setName(e.target.value)} />}
							<Input id='email' type='email' label='Email address or phone number' value={email} onChange={(e: any) => setEmail(e.target.value)} />
							<Input type='password' id='password' label='Password' value={password} onChange={(e: any) => setPassword(e.target.value)} />
						</div>
